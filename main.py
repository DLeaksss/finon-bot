...
async def on_command_error(ctx, error):
	if isinstance(error, commands.CommandOnCooldown):
		...
		report_error_channel = client.get_channel(997545537681236128)
...
async def crash(ctx):
	webhook = client.get_channel(997545410052751360)
...
async def report(ctx, *, описание: str):
	if ctx.author.id not in чёрныйлистик:
		if ctx.author.id not in no_report:
			finontest = client.get_channel(997545641146322944)
			cpage = discord.Embed(
